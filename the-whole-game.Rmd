...
```{r mutate-accross-where-filter-1}
...
```{r mutate-accross-where-filter-2, eval=FALSE}
______2 <- messy %>% 
  mutate(______(where(is._______), round)) %>% 
  ______(continent %in% params$__________)

messy2
```

```{r mutate-accross-where-filter-3}
messy2 <- messy %>% 
