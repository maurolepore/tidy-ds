...
To tidy this dataset we pivot over all columns except `country`. We may exclude `country` with `-` or select all other columns with `where(is.numeric)`L

```{r}
longer <- toy %>% pivot_longer(cols = where(is.numeric))
longer

# Same
longer1 <- toy %>% pivot_longer(cols = where(is.numeric))
identical(longer1, longer)
```
